  ...
  return (
    <ChartWrapper title={title} subtitle={subtitle}>
      <div className="relative h-full">
        {/* GEÄNDERT: Y-Achsen-Labels auf Slate-Theme umgestellt */}
        ...
        <div className="ml-16 h-full pb-8">
          <svg className="w-full h-full" viewBox="0 0 100 100" preserveAspectRatio="none">
            {/* GEÄNDERT: Grid-Linien auf Slate-Farben umgestellt */}
          ...
          {/* GEÄNDERT: X-Achsen-Labels auf Slate-Theme umgestellt */}
          <div className="flex justify-between text-xs text-slate-500 mt-2">
            {data
              .filter(
                (_, i) =>
                  i % Math.ceil((data.length || 1) / 5) === 0 || i === data.length - 1
              )
              .map((d, i) => (
